...
static uint16_t ReadDumpRam16(const uint8_t *ram, size_t ram_size,
                              size_t offset) {
  return offset + 1 < ram_size ?
         (uint16_t)(ram[offset] | ram[offset + 1] << 8) : 0;
}
...
bool Platform3DS_DumpMemory(const char *directory,
  ...
  if (info) {
    ...
    if (ram_size >= 0x620) {
      fprintf(info, "Module: %u.%u\n", ram[0x10], ram[0x11]);
      fprintf(info, "Indoor: %u\n", ram[0x1b]);
      fprintf(info, "Overworld screen/overlay: %04x/%04x\n",
              ReadDumpRam16(ram, ram_size, 0x8a),
              ReadDumpRam16(ram, ram_size, 0x8c));
      fprintf(info, "Link position: %u,%u\n",
              ReadDumpRam16(ram, ram_size, 0x22),
              ReadDumpRam16(ram, ram_size, 0x20));
      fprintf(info, "BG2 scroll: %u,%u\n",
              ReadDumpRam16(ram, ram_size, 0xe2),
              ReadDumpRam16(ram, ram_size, 0xe8));
