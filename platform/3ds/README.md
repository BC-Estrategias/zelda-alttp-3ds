...
- Settings > Screen > Edge Mode: `FIXED CAMERA` stops camera movement before
  a wide-mode boundary; `STANDARD` uses the original v1.8 camera behavior.
...
The HOME Menu metadata is versioned for every release. v2.1 uses:

```text
Short name: Zelda ALttP 3DS
Long name:  Zelda A Link to the Past 3DS v2.1
```
